...
configure_basic_logging("worldbank")
logger = get_ingestion_logger("worldbank")
_tracer = get_tracer("parsnip.ingestion.worldbank")
...
async def process_records(records: list[dict]):
    ...
    start_metrics_server()
    with _tracer.start_as_current_span("ingest_worldbank") as span:
        span.set_attribute("source", "world_bank")
    conn = None
    job_id = None
    try:
        ...
        INGESTION_CHUNKS_PROCESSED.labels(source="world_bank", status="success").inc(total_kb)
        ...
        INGESTION_JOBS_FAILED.labels(source="world_bank").inc()
        if conn is not None and job_id is not None:
            try:
                await write_to_dlq(conn, source="world_bank", source_id=f"job:{job_id}",
                                   content=None, metadata={"job_id": job_id}, error=exc)
                INGESTION_DLQ_MESSAGES.labels(source="world_bank").inc()
                await emit_dlq_lineage(conn, source="world_bank", job_id=job_id)
